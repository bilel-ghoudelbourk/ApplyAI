...
type SearchByCVPanelProps = {
    cvCacheId: string;
    initialMetadata?: CVMetadata;
};
...
export function SearchByCVPanel({ cvCacheId, initialMetadata }: SearchByCVPanelProps) {
    const { t, lang } = useLanguage();
    const { theme } = useTheme();
    
    // We keep keywords internal but hidden from UI as per previous instructions
    const [keywords, setKeywords] = useState(initialMetadata?.suggested_keywords || "");
    ...
    // Sync if initialMetadata changes (e.g. new upload)
    useEffect(() => {
        if (initialMetadata) {
            if (!keywords) setKeywords(initialMetadata.suggested_keywords || "");
            if (!location) setLocation(initialMetadata.location || "");
        }
    }, [initialMetadata, keywords, location]);

    const handleRefreshSuggestions = () => {
        if (initialMetadata) {
            setKeywords(initialMetadata.suggested_keywords || "");
            setLocation(initialMetadata.location || "");
    ...
    const handleSearch = async () => {
        if (sources.length === 0) {
            alert(t("searchFailed") + ": Please select at least one source.");
            return;
        }
        ...
        try {
            ...
            const resp = await searchByCV(
                cvCacheId,
                trimmedKeywords || null,
            ...
            console.error(err);
            setSearchFeedback(null);
            alert(t("searchByCVFailed") || t("searchFailed"));
        } finally {
    ...
    return (
        <motion.div
            key="search-cv-inputs"
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.95 }}
            className="w-full flex flex-col gap-5"
        >
            {/* Row 1: Location (Takes full width since keywords are hidden) */}
                ...
                >
                    <div className="flex items-center justify-between mb-2 ml-2">
                        <label
                            ...
                            style={{ color: "var(--text-primary)" }}
                        >
                            <MapPin className="w-3.5 h-3.5 text-neon-purple" /> {t("searchLocationLabel")} ({t("optional")})
                        </label>
                        {initialMetadata && (
                            <button
                                onClick={handleRefreshSuggestions}
                                className="p-1 rounded hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
                                title="Reset to CV suggestions"
                                style={{ color: "var(--text-muted)" }}
                            >
                    ...
                    <input
                        ...
                        placeholder={t("searchLocationPlaceholder") + " (" + t("optional") + ")"}
            ...
            {/* Row 2: Date & Limit (Same as SearchJobsPanel) */}
            <div className="flex flex-col md:flex-row gap-4">
                <div
            ...
            {/* Row 3: Sources (Same as SearchJobsPanel) */}
            <div
            ...
            <button
                ...
                disabled={isSearching}
                className={cn(
                    "mt-2 w-full font-bold disabled:opacity-50 p-4 rounded-xl transition-all flex items-center justify-center gap-3 hover:opacity-90",
                    theme === "dark"
